...
export class TripEditComponent {
  modelForm: FormGroup = this.formBuilder.group({
    ...
    price: new FormControl('', Validators.pattern("^[1-9]{1}[0-9]{0,}$")),
    quantity: new FormControl('', Validators.pattern("^[1-9]{1}[0-9]{0,}$")),
  ...
  constructor(private formBuilder : FormBuilder, firebaseService: FirestoreServiceService, private route: ActivatedRoute, public authService: AuthService, public router: Router){
  ...
  onSubmit(): void{
    let newTrip: INewTrip = {
      id: this.trip.id,
      name: this.modelForm.value.name == null ? this.trip.name : this.modelForm.value.name,
      destination: this.modelForm.value.destination == null ? this.trip.destination : this.modelForm.value.destination,
      start: this.modelForm.value.start == null ? this.trip.start : this.modelForm.value.start.substr(8,2) + '-' + this.modelForm.value.start.substr(5,2) + '-' + this.modelForm.value.start.substr(0,4),
      end: this.modelForm.value.end == null ? this.trip.end : this.modelForm.value.end.substr(8,2) + '-' + this.modelForm.value.end.substr(5,2) + '-' + this.modelForm.value.end.substr(0,4),
      price: this.modelForm.value.price == null ? this.trip.price : parseInt(this.modelForm.value.price, 10),
      quantity: this.modelForm.value.quantity == null ? this.trip.quantity : parseInt(this.modelForm.value.quantity, 10),
      description: this.modelForm.value.description == null ? this.trip.description : this.modelForm.value.description,
      images: [this.modelForm.value.image == null ? this.trip.images[0] : this.modelForm.value.image, this.modelForm.value.image2 == null ? this.trip.images[1] : this.modelForm.value.image2, this.modelForm.value.image3 == null ? this.trip.images[2] : this.modelForm.value.image3],
      numOfReviews: this.trip.numOfReviews,
      rating: this.trip.rating,
      reviews: this.trip.reviews
    }
    this.firebaseService.editTrip(newTrip);
    this.modelForm.reset();
